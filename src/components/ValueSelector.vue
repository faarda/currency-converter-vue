  ...
  <div class="converter__main">
    <div class="converter__main__top">
      <div class="converter__main__value" @click="openCurrencyPicker('from')">
        ...
        <h1
          class="converter__main__value__value scaling-text"
          :style="{ '--length': getDigitsLength(converter.from.value) }"
        >
    ...
    <div class="converter__main__bottom">
      <div class="converter__main__value" @click="openCurrencyPicker('to')">
        ...
        <h1
          class="converter__main__value__value scaling-text"
          :style="{ '--length': getDigitsLength(converter.to.value) }"
        >
...
export default {
  ...
  watch: {
    //fetch currency data from store when any of the currencies changes
    converter: {
      immediate: true,
      deep: true,
      handler(value) {
        this.fromCurrency = getCurrencyFromCode(
          this.currencies,
          value.from.currency
        );
        this.toCurrency = getCurrencyFromCode(
          this.currencies,
          value.to.currency
        );
      },
