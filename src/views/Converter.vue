<template>
  <div class="converter">
    <value-selector
      @open-picker="togglePicker($event)"
      :converter="converter"
    ...
    <currency-picker
      :converter="converter"
      :picking="picking"
      :pickerOpen="isCurrencyPickerOpen"
      @close-picker="togglePicker()"
      :currencies="currencies"
      @currency-picked="switchRouteCurrency($event)"
    />
    <div class="converter__error" v-if="isCurrencyInvalid">
      <h6>
        Currency combination not supported, supported currencies are
        {{ currenciesToCommaSeparatedList(currencies) }}
      </h6>
      <router-link class="converter__error__link" to="/"
...
export default {
      ...
      if (!isNaN(key)) {
        this.rawInputValue =
          this.rawInputValue === 0
            ? key
            : Number(`${this.rawInputValue}${key}`);
      } else if (key === "clear") {
        this.rawInputValue = 0;
      } else {
        this.$router.push(
          `/convert/${this.selectedCurrencies.to}/${this.selectedCurrencies.from}`
        );
      }
    });
  },
  computed: {
    ...
    // use computed data to keep track of currency & converted values
    converter() {
    ...
    conversionRate() {
      ...
      return (
        this.baseEurRates[this.selectedCurrencies.to] /
        this.baseEurRates[this.selectedCurrencies.from]
      );
    },
  },
  methods: {
    //opens and closes the currency picker
    ...
    //gets the selected currency codes from the route and performs neccesary checks
    getSelectedCurrencies() {
      ...
      if (
        currencyCodeList.includes(from.toUpperCase()) &&
        currencyCodeList.includes(to.toUpperCase())
      ) {
    ...
    //changes routes when a new currency is picked
    switchRouteCurrency(data) {
      const toCurrency =
        data.where === "to" ? data.currency : this.selectedCurrencies.to;
      const fromCurrency =
        data.where === "from" ? data.currency : this.selectedCurrencies.from;

      //avoid duplicate currencies
      if (toCurrency !== fromCurrency) {
        //check if previous & current currency are similar to avoid redirecting to the same route
        if (
          toCurrency !== this.selectedCurrencies.to ||
          fromCurrency !== this.selectedCurrencies.from
        ) {
